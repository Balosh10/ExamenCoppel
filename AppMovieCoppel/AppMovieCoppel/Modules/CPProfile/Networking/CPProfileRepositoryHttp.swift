...
class CPProfileRepositoryHttp: CPProfileRepository {
    ...
    func fetchMoviesFavorite(_ completion: @escaping (Result<CPMovies, NSError>) -> Void) {
        ...
        let endPoint = "account/1/favorite/movies?sort_by=created_at.desc&language=es-MX&api_key=\(Setting.apiKey)&session_id=\(sesionId)"
    ...
    func fetchAccount(_ completion: @escaping (Result<CPAccount, NSError>) -> Void) {
        services = APIService()
        let endPoint = "account?api_key=\(Setting.apiKey)&session_id=\(CPSession.shared.getStringValue(.sessionId))"
        services?.apiRequest(endPoint,
                             CPAccount.self,
                             .get) { result in
            switch result {
